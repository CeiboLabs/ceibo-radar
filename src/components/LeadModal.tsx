...
export function LeadModal({ lead, onClose, onUpdate, onDelete }: LeadModalProps) {
  ...
  // Tab state
  const [activeTab, setActiveTab] = useState<"negocio" | "gestion" | "analisis">("negocio");
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          {(() => {
            ...
            return (
              <div className="sticky top-0 z-10 bg-gray-900 border-b border-gray-800 px-6 py-3 -mx-6 -mt-5 mb-5 flex gap-1">
                <button className={tabClass("negocio")} onClick={() => setActiveTab("negocio")}>Negocio</button>
                <button className={tabClass("gestion")} onClick={() => setActiveTab("gestion")}>Gestión</button>
                <button className={tabClass("analisis")} onClick={() => setActiveTab("analisis")}>Análisis</button>
              </div>
            );
          })()}
        ...
        <div className="sticky bottom-0 bg-gray-900/95 backdrop-blur border-t border-gray-800 px-6 py-4 flex gap-3 shrink-0 flex-wrap">
          ...
          {onDelete && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-gray-800 hover:bg-red-950 text-gray-500 hover:text-red-400 rounded-lg text-sm transition-colors border border-gray-700 hover:border-red-900"
              title="Eliminar lead"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
